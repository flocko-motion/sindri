...
// The file-sink access log shows every distinct request once and collapses a repeated
// endpoint into a single "(×N)" summary instead of dropping or flooding.
...
// LogRequests never records the SSE stream (it would hold a run open for the life of
// the connection).
