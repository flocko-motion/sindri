...
func (p *ProjectStore) PutRun(r Run) error {
	...
	_, err := p.s.db.Exec(`
		INSERT INTO runs (project,id,agent,command,status,priority,timeout,workspace,task,kind,message,commit_sha,created_at,started_at,finished_at,updated_at)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
		ON CONFLICT(project,id) DO UPDATE SET
			...
			task=excluded.task, kind=excluded.kind, message=excluded.message, commit_sha=excluded.commit_sha,
			started_at=excluded.started_at,
			finished_at=excluded.finished_at, updated_at=excluded.updated_at`,
		p.project, r.ID, r.Agent, r.Command, r.Status, r.Priority, r.Timeout, r.Workspace, r.Task,
		r.Kind, r.Message, r.Commit, r.CreatedAt, r.StartedAt, r.FinishedAt, r.UpdatedAt)
	if err != nil {
...
const runCols = `SELECT project,id,agent,command,status,priority,timeout,workspace,task,exit_code,kind,message,commit_sha,created_at,started_at,finished_at,updated_at FROM runs`
...
func scanRunRow(row scanner) (Run, error) {
	...
	err := row.Scan(&r.Project, &r.ID, &r.Agent, &r.Command, &r.Status, &r.Priority, &r.Timeout,
		&r.Workspace, &r.Task, &r.ExitCode, &r.Kind, &r.Message, &r.Commit, &r.CreatedAt, &r.StartedAt,
