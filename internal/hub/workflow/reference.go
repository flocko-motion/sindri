...
func (e *Engine) referenceMoved(project, root, base, prevTip, tip string, advanced bool) {
	...
	for _, a := range roster {
		...
		case !advanced:
			...
			_ = e.hn.Say(project, a.Name, MsgReferenceRewritten(), MailAndPush)
		case underReview:
...
func (e *Engine) advanceAgent(project, root, base, prevTip, tip string, a store.Agent) {
	...
	if rebaseErr != nil {
		...
		_ = e.hn.Say(project, a.Name, MsgReferenceNeedsRebase(incoming), MailAndPush)
		return
	}
	_ = ps.Log(a.Name, "reference-advanced", "rebased onto "+base)
	_ = e.hn.Say(project, a.Name, MsgReferenceAdvanced(incoming), PushOnly)
}
