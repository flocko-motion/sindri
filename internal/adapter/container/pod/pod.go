...
// It forwards the caller's TERM/COLORTERM (podman exec otherwise drops the host env),
// so the tmux attach client can drive the real terminal — without it the pod's empty
// TERM leaves cursor addressing/colour broken and the session renders scrambled.
