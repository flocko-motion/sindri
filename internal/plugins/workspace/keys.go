...
import (
	...
	appmsg "github.com/flo-at/sindri/internal/msg"
	"github.com/flo-at/sindri/internal/plugins/gitstatus"
	"github.com/flo-at/sindri/internal/state"
)

// handleInputModeKeys handles keys while in input mode (vim "i" insert).
// Esc cancels, Enter submits the message as a td comment.
func (p *Plugin) handleInputModeKeys(msg tea.KeyMsg) tea.Cmd {
	switch msg.Type {
	case tea.KeyEsc:
		p.inputMode = false
		return nil
	case tea.KeyEnter:
		text := p.inputField.Value()
		p.inputMode = false
		if text == "" {
			return nil
		}
		...
		return p.sendInputToAgent(wt, text)
	default:
		var cmd tea.Cmd
		p.inputField, cmd = p.inputField.Update(msg)
		return cmd
	}
}

// sendInputToAgent sends a message to the agent's Claude session inside the container.
func (p *Plugin) sendInputToAgent(wt *Worktree, input string) tea.Cmd {
	return func() tea.Msg {
		container := containerNameFor(wt.Name, wt.IsMain)
		// Resume the background session with the new message
		cmd := exec.Command("podman", "exec", container,
			"claude", "--dangerously-skip-permissions", "-c", "-p", input)
		out, err := cmd.CombinedOutput()
		if err != nil {
			return app.ToastMsg{Message: "Send failed: " + strings.TrimSpace(string(out)), Duration: 3 * time.Second, IsError: true}
		}
		return app.ToastMsg{Message: "→ " + wt.Name, Duration: 1 * time.Second}
	}
}

// handleKeyPress processes key input based on current view mode.
func (p *Plugin) handleKeyPress(msg tea.KeyMsg) tea.Cmd {
	// Input mode: all keys go to the text field
	if p.inputMode {
		return p.handleInputModeKeys(msg)
	}
...
func (p *Plugin) handleListKeys(msg tea.KeyMsg) tea.Cmd {
	...
	case "n":
		...
		// Auto-create a new worker with a Norse name
		return p.quickCreateWorker()
	case "i":
		// Input mode — type a line to send to the container's stdin
		p.inputMode = true
		p.inputField = textinput.New()
		p.inputField.Placeholder = "send to agent..."
		p.inputField.Prompt = "› "
		p.inputField.Focus()
		p.inputField.CharLimit = 500
		return nil
	case "D":
	...
	case "enter":
		...
		// Launch container for workspaces without a running worker
		if !p.shellSelected {
			wt := p.selectedWorktree()
			if wt != nil && wt.Health != HealthHealthy && wt.Health != HealthStarting && wt.Health != HealthCreating {
				wt.Health = HealthStarting
				wt.ContainerStatus = "launching..."
				// Launch and start log stream in parallel
				return tea.Batch(
					launchSindriContainer(p.ctx.ProjectRoot, wt),
					startLogStream(wt.Name, wt.IsMain),
				)
			}
		}
	case "t":
	...
	// Agent control keys
	case "S":
		// Stop container for selected worktree
		wt := p.selectedWorktree()
		if wt != nil && wt.Health == HealthHealthy {
			containerName := "sindri-" + wt.Name
			if wt.IsMain {
				containerName = "sindri-reviewer"
			}
			wt.Health = HealthNoWorker
			return func() tea.Msg {
				_ = exec.Command("podman", "stop", containerName).Run()
				_ = exec.Command("podman", "rm", containerName).Run()
				return app.ToastMsg{Message: "Stopped " + wt.Name, Duration: 2 * time.Second}
			}
		}
	case "K":
