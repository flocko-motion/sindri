...
import (
	...
	"github.com/flo-at/sindri/internal/markdown"
	"github.com/flo-at/sindri/internal/modal"
	"github.com/flo-at/sindri/internal/mouse"
	"github.com/flo-at/sindri/internal/plugin"
	"github.com/flo-at/sindri/internal/projectdir"
	"github.com/flo-at/sindri/internal/plugins/gitstatus"
	"github.com/flo-at/sindri/internal/state"
	"github.com/flo-at/sindri/internal/ui"
)
...
type Plugin struct {
	...
	// Input mode ("i" to activate, Esc to cancel, Enter to send)
	inputMode  bool
...
func (p *Plugin) Start() tea.Cmd {
	...
	// Initial podman container status poll
...
// mainWorktreeName returns the name of the main worktree, or empty string.
func (p *Plugin) mainWorktreeName() string {
	for _, wt := range p.worktrees {
		if wt.IsMain {
			return wt.Name
		}
...
// quickCreateWorker creates a new worktree with an auto-generated Norse name
// and immediately launches a Sindri agent in it. No modal, no prompts.
func (p *Plugin) quickCreateWorker() tea.Cmd {
	// Check if repo has commits (can't create worktree from empty repo)
	checkCmd := exec.Command("git", "-C", p.ctx.ProjectRoot, "rev-parse", "HEAD")
	if err := checkCmd.Run(); err != nil {
		p.errorMessage = "Cannot create worker: repo has no commits yet. Commit something first."
	...
	var taken []string
	for _, wt := range p.worktrees {
		taken = append(taken, wt.Name)
	}
	name := nextNorseName(taken)

	// Add placeholder to list immediately so user sees feedback
	placeholder := &Worktree{
		Name:   name,
		Branch: name,
		Health: HealthCreating,
	}
	p.worktrees = append(p.worktrees, placeholder)
	p.shellSelected = false
	p.selectedIdx = len(p.worktrees) - 1
	p.ensureVisible()

	return func() tea.Msg {
		wt, err := p.doCreateWorktree(name, "HEAD", "", "", AgentSindri)
		return CreateDoneMsg{Worktree: wt, AgentType: AgentSindri, SkipPerms: true, Err: err}
	}
}
...
func (p *Plugin) openCreateModal() tea.Cmd {
	...
	// Auto-generate a Norse name for the worker
	var taken []string
	for _, wt := range p.worktrees {
		taken = append(taken, wt.Name)
	}
	name := nextNorseName(taken)
	p.createNameInput.SetValue(name)
	p.branchNameValid, p.branchNameErrors, p.branchNameSanitized = ValidateBranchName(name)

	// Always use Sindri agent
	p.createAgentType = AgentSindri
