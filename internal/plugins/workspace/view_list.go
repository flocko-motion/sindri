...
func (p *Plugin) View(width, height int) string {
	...
	case ViewModeError:
		return p.renderErrorModal(width, height)
...
func (p *Plugin) renderListView(width, height int) string {
	...
	if p.inputMode {
		previewHeight = innerHeight - 1
	}
	previewContent := p.renderPreviewContent(previewContentW, previewHeight)
	if p.inputMode {
		inputBar := styles.Muted.Render("INPUT ") + p.inputField.View()
...
func (p *Plugin) renderSidebarContent(width, height int) string {
	...
	// Shells disabled — full height for worktrees
...
func (p *Plugin) renderWorktreeItem(wt *Worktree, selected bool, width int) string {
	...
	// Icon: crown for reviewer, hammer for workers
	...
	case HealthOrphanedWorker:
		healthStyle = lipgloss.NewStyle().Foreground(styles.Error)
	default:
		healthStyle = styles.Muted
	}
	// Check for conflicts
	...
	// Stats if available
	statsStr := ""
	if !wt.IsMain && !sdCfg.HideStats && wt.Stats != nil && (wt.Stats.Additions > 0 || wt.Stats.Deletions > 0) {
		statsStr = fmt.Sprintf("+%d -%d", wt.Stats.Additions, wt.Stats.Deletions)
	}

	// Status line — shared between selected and non-selected rendering
	role := "worker"
	if wt.IsMain {
		role = "reviewer"
	}
	...
	case HealthHealthy:
		healthText = "running"
	case HealthNoWorker:
		healthText = "no container"
	case HealthStarting:
		healthText = wt.ContainerStatus
	case HealthOrphanedWorker:
		healthText = "orphaned"
	case HealthCreating:
		healthText = "creating..."
	...
	// When selected, preserve health color on the icon
	if isSelected {
		coloredIcon := healthStyle.Render(statusIcon)
		line1 := fmt.Sprintf(" %s %s%s%s%s", coloredIcon, name, prIcon, conflictIcon, orphanedIcon)
	...
	// Not selected - color based on worker health
	icon := healthStyle.Render(statusIcon)
	...
	// For non-selected, colorize the health portion
	var styledParts []string
	styledStatus := styles.Muted.Render(role+" · ") + healthStyle.Render(healthText)
	styledParts = append(styledParts, styledStatus)
	if wt.TaskID != "" {
