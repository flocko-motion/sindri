...
// renderPreviewContent renders the unified three-panel preview:
// top: task bar, middle: agent output, bottom: input (when active).
func (p *Plugin) renderPreviewContent(width, height int) string {
	...
	// === Task bar (1 line) ===
	taskBar := p.renderTaskBar(wt, width)

	// === Separator ===
	sep := styles.Muted.Render(strings.Repeat("─", width))

	// === Output (fills remaining space) ===
	outputHeight := height - 2 // subtract task bar + separator

	var output string
	switch wt.Health {
	case HealthHealthy:
		output = p.renderOutputContent(width, outputHeight)
	case HealthStarting, HealthCreating:
		msg := lipgloss.NewStyle().Foreground(styles.Warning).Render("Launching agent...")
		output = lipgloss.Place(width, outputHeight, lipgloss.Center, lipgloss.Center, msg)
	case HealthNoWorker, HealthUnknown:
		msg := styles.Muted.Render("No agent running") + "\n" + lipgloss.NewStyle().Foreground(styles.Success).Render("Press Enter to launch")
		output = lipgloss.Place(width, outputHeight, lipgloss.Center, lipgloss.Center, msg)
	default:
		msg := lipgloss.NewStyle().Foreground(styles.Error).Render("Agent error")
		output = lipgloss.Place(width, outputHeight, lipgloss.Center, lipgloss.Center, msg)
	}
...
// renderTaskBar renders the single-line task status bar at the top of the preview.
func (p *Plugin) renderTaskBar(wt *Worktree, width int) string {
	icon := "🔨"
	role := "worker"
	if wt.IsMain {
		icon = "👑"
	...
	taskInfo := styles.Muted.Render("no task")
	if wt.TaskID != "" {
		taskInfo = wt.TaskID
		if wt.TaskTitle != "" {
			taskInfo += " " + styles.Muted.Render(wt.TaskTitle)
		}
	}

	bar := fmt.Sprintf(" %s %s · %s", icon, role, taskInfo)
...
func (p *Plugin) renderOutputContent(width, height int) string {
	...
	if wt.PodmanLogs == "" {
		return dimText("Waiting for output...")
	}

	// Show the tail of podman logs, fitting the available height
	logLines := strings.Split(wt.PodmanLogs, "\n")
	start := 0
	if len(logLines) > height {
		start = len(logLines) - height
	}
