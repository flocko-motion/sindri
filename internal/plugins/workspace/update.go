...
import (
	...
	app "github.com/flo-at/sindri/internal/app"
	"github.com/flo-at/sindri/internal/migration"
...
func (p *Plugin) Update(msg tea.Msg) (plugin.Plugin, tea.Cmd) {
	...
	case RefreshDoneMsg:
		...
		if msg.Err == nil {
			...
			// Sindri uses podman containers, not tmux — skip tmux reconnect
			p.initialReconnectDone = true
		}

	case PodmanStatusMsg:
		if msg.Err == nil {
			p.containers = msg.Containers
			// Update health status for each worktree
			for _, wt := range p.worktrees {
				prevHealth := wt.Health
				if wt.IsMain {
					if has, status := workerHasContainer(p.containers, "_reviewer"); has {
						wt.Health = HealthHealthy
						wt.ContainerStatus = status
					} else {
						wt.Health = HealthNoWorker
					}
				} else if has, status := workerHasContainer(p.containers, wt.Name); has {
					if status == "running" {
						wt.Health = HealthHealthy
					} else {
				...
				// Start log stream when container just became healthy
				if wt.Health == HealthHealthy && prevHealth != HealthHealthy {
					if _, streaming := activeLogStreams.Load(wt.Name); !streaming {
						cmds = append(cmds, startLogStream(wt.Name, wt.IsMain))
					}
				}
			}
		}
		...
		cmds = append(cmds, tea.Tick(5*time.Second, func(t time.Time) tea.Msg {
			return podmanPollTickMsg{}
		}))

	case podmanPollTickMsg:
		cmds = append(cmds, pollPodmanStatus(p.ctx.ProjectRoot))

	case PodmanLogsMsg:
		for _, wt := range p.worktrees {
			if wt.Name == msg.WorkerName {
				wt.PodmanLogs += msg.Lines
				break
			}
		}
		// Keep reading from the stream
		cmds = append(cmds, waitForLogChunk(msg.WorkerName))

	case PodmanLogStreamDoneMsg:
		// Stream ended — container likely stopped
		for _, wt := range p.worktrees {
			if wt.Name == msg.WorkerName {
				wt.PodmanLogs += "\n[stream ended]"
				break
	...
	case SindriLaunchMsg:
		if msg.Err != nil {
			p.errorMessage = "Failed to launch " + msg.WorkerName + ": " + msg.Err.Error()
			p.viewMode = ViewModeError
			// Remove placeholder on failure
			for i, wt := range p.worktrees {
				if wt.Name == msg.WorkerName && wt.Health == HealthStarting {
					wt.Health = HealthNoWorker
					_ = i
					break
				}
	...
	case CreateDoneMsg:
		if msg.Err != nil {
			...
			p.errorMessage = "Worker creation failed: " + msg.Err.Error()
			p.viewMode = ViewModeError
			// Remove placeholder on failure
			for i, wt := range p.worktrees {
				if wt.Health == HealthCreating {
			...
			// Replace placeholder with real worktree, or append if no placeholder found
			replaced := false
			for i, wt := range p.worktrees {
				if wt.Health == HealthCreating && wt.Name == msg.Worktree.Name {
					p.worktrees[i] = msg.Worktree
					p.selectedIdx = i
					replaced = true
					break
				}
			...
			cmds = append(cmds, func() tea.Msg {
				return app.ToastMsg{
					Message:  "Worker " + msg.Worktree.Name + " created",
					Duration: 3 * time.Second,
				}
			})

			// Launch Sindri container and start streaming in parallel
			if msg.AgentType != AgentNone && msg.AgentType != "" {
				cmds = append(cmds,
					launchSindriContainer(p.ctx.ProjectRoot, msg.Worktree),
					startLogStream(msg.Worktree.Name, msg.Worktree.IsMain),
				)
			}
		}
