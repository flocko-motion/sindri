...
const (
	...
	ViewModeError                          // Error display modal
)
...
const (
	AgentNone   AgentType = ""       // No agent
	AgentSindri AgentType = "sindri" // Sindri worker agent
	AgentClaude AgentType = "claude" // Legacy alias for Sindri
	AgentShell  AgentType = "shell"  // Legacy: unused
)

var SkipPermissionsFlags = map[AgentType]string{
	AgentSindri: "--dangerously-skip-permissions",
...
// WorkerHealth represents the health state of a workspace+worker pair.
type WorkerHealth int

const (
	HealthUnknown          WorkerHealth = iota
	HealthHealthy                       // Workspace exists + container running
	HealthNoWorker                      // Workspace exists, no container
	HealthOrphanedWorker                // Container running, no workspace
	HealthStarting                      // Container exists but not yet running
	HealthCreating                      // Worktree being created
)

type Worktree struct {
	Name            string         // e.g., "brokkr"
	...
	Health          WorkerHealth   // Derived from podman container state
	ContainerStatus string         // Raw podman status ("running", "exited", etc.)
	Stats           *GitStats      // +/- line counts
	CreatedAt       time.Time
	UpdatedAt       time.Time
	IsOrphaned      bool // True if container is gone but workspace remains
	IsMain          bool   // True if this is the primary/main worktree (project root)
	IsMissing       bool   // True if worktree directory no longer exists
	PodmanLogs      string // Cached container log output
}
