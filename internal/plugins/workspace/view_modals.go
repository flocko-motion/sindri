...
// renderErrorModal renders a centered error message over the list view.
func (p *Plugin) renderErrorModal(width, height int) string {
	background := p.renderListView(width, height)
	...
	modalContent := errorStyle.Render(content)
	return ui.OverlayModal(background, modalContent, width, height)
}
