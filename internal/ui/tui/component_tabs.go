...
func headerBar(labels []string, active, width int, repoName, repoTag string, repoColor int, mem api.FleetMemory, running, existing int) string {
	if repoName == "" {
		return plainTabStrip(labels, active, width, mem, running, existing)
	}
	...
	ind := "◉ " + repoName + " "
	badge := headroomBadge(mem, running, existing, width-plainW-lipgloss.Width(ind))
...
func plainTabStrip(labels []string, active, width int, mem api.FleetMemory, running, existing int) string {
	...
	badge := headroomBadge(mem, running, existing, width-lipgloss.Width(strip))
...
func headroomBadge(mem api.FleetMemory, running, existing, space int) string {
	badge := theme.FleetBadge(mem, running, existing, space-2)
	if badge == "" {
		return ""
	}
	return " " + badge + " "
