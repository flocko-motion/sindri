...
func (s stubTaskInfoBackend) TaskInfo(string) (store.Task, error) { return s.task, s.err }
...
func TestAgentTaskLabel(t *testing.T) {
	cases := []struct {
		...
		{"title found", "td-42", stubTaskInfoBackend{task: store.Task{Title: "fix the flaky test"}}, "td-42  fix the flaky test"},
		{"lookup fails", "td-42", stubTaskInfoBackend{err: errors.New("no such task")}, "td-42"},
		{"title empty", "td-42", stubTaskInfoBackend{task: store.Task{}}, "td-42"},
	}
	for _, c := range cases {
