...
func (p *Plugin) renderWorktreeItem(wt *Worktree, selected bool, width int) string {
	...
	// Health indicator based on podman container state
	...
	if wt.IsMain {
		...
		case HealthNoWorker:
			parts = append(parts, "no container")
		case HealthStarting:
			parts = append(parts, wt.ContainerStatus)
		case HealthOrphanedWorker:
			parts = append(parts, "orphaned container")
		default:
	...
	if wt.IsMain {
		styledParts = append(styledParts, styles.Muted.Render("reviewer"))
	} else {
		switch wt.Health {
		case HealthHealthy:
			styledParts = append(styledParts, lipgloss.NewStyle().Foreground(styles.Success).Render("running"))
		case HealthNoWorker:
			styledParts = append(styledParts, lipgloss.NewStyle().Foreground(styles.Error).Render("no container"))
		case HealthStarting:
			styledParts = append(styledParts, lipgloss.NewStyle().Foreground(styles.Warning).Render(wt.ContainerStatus))
		case HealthOrphanedWorker:
			styledParts = append(styledParts, lipgloss.NewStyle().Foreground(styles.Error).Render("orphaned"))
		default:
			styledParts = append(styledParts, styles.Muted.Render("unknown"))
		}
	}
	if wt.TaskID != "" {
