...
func (p *Plugin) Update(msg tea.Msg) (plugin.Plugin, tea.Cmd) {
	...
	case PodmanStatusMsg:
		if msg.Err == nil {
			...
			for _, wt := range p.worktrees {
				if wt.IsMain {
					// Main repo is the reviewer — check for reviewer container
				...
				if has, status := workerHasContainer(p.containers, wt.Name); has {
					if status == "running" {
						wt.Health = HealthHealthy
					} else {
	...
	case SindriLaunchMsg:
		if msg.Err != nil {
			...
			cmds = append(cmds, func() tea.Msg {
				return app.ToastMsg{
					Message:  "Worker " + msg.WorkerName + " launched",
					Duration: 3 * time.Second,
				}
			})
			// Trigger immediate podman poll to update health status
			cmds = append(cmds, pollPodmanStatus(p.ctx.ProjectRoot))
		}
	...
	case CreateDoneMsg:
		if msg.Err != nil {
			...
			// Launch Sindri container for the new worker
			if msg.AgentType != AgentNone && msg.AgentType != "" {
				cmds = append(cmds, launchSindriContainer(p.ctx.ProjectRoot, msg.Worktree))
			}
		}
