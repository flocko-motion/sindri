...
func detectAgentSessionStatus(agentType AgentType, worktreePath string) (WorktreeStatus, bool) {
	switch agentType {
	case AgentSindri, AgentClaude:
		return detectSindriSessionStatus(worktreePath)
	default:
		return 0, false
	}
}

// detectSindriSessionStatus checks the Sindri agent's session state.
// Currently delegates to Claude Code's session detection since Sindri wraps Claude.
func detectSindriSessionStatus(worktreePath string) (WorktreeStatus, bool) {
