...
- [x] 1.1 `store.Task` gains `ParentID`, `Description`, `Acceptance`; add the
      columns to the `tasks` table; `ReplaceTasks`/`UpsertTask`/`scanTasks` carry them
- [x] 1.2 `adapter/td/sqlite.go`: select `parent_id, description, acceptance`;
      `toStoreTask`/`scanDBTask` map them
- [x] 1.3 Board carries **all** tasks (sync with `issue.FilterAll`), each with
      parent + description; `BoardState.Tasks` = `store.AllTasks()`. Section count
      derives the non-closed subset
- [x] 1.4 Confirm a task resolves its working agent (assignee) from the board

## 2. Section model + tree + cross-links (hub)

- [x] 2.1 `hub.Section{Key,Title,Count func(BoardState) int}` + ordered `Sections`
      (tasks=non-closed, agents=running, prs=not-merged)
- [x] 2.2 `hub.TaskRow{store.Task; Depth int; PR string}` +
      `ArrangeTasks(tasks, prs) []TaskRow` (tree by parent, priority-sorted roots,
      absent-parent→root, PR-annotated)
- [x] 2.3 `PRInfo` → `PRDetail{PR, Task store.Task, Diff}` (fetch task via td.Get)
- [x] 2.4 Unit tests: section counts, ArrangeTasks ordering/depth/PR-mark/orphan-parent

## 3. Scroll primitive (build + test first — everything renders through it)

- [x] 3.1 `internal/tui/scroll.Viewport` — pure value type (`Height`, `Total`,
...
- [x] 3.2 Exhaustive unit tests: content shorter than pane (pad), longer (scroll),
      cursor at edges, resize below cursor, content shrink below offset, page near
      ends, total<height

## 4. TUI rewrite (internal/tui)

- [x] 4.1 Full-height master-detail frame from `WindowSizeMsg`: tab strip (row 0),
      body (h-3), footer (last 2 rows); left selector + divider + detail pane,
      each a fixed-height `scroll.Viewport`
- [x] 4.2 Tab strip from `hub.Sections` with `[<n> <Title>]` live counts; active
      tab highlighted
- [x] 4.3 vi nav: `ctrl+h/l`+`1/2/3` tabs; `j/k`,`g/G`,`ctrl+d/u` move (drives the
      selector viewport's cursor); selection drives the detail pane
- [x] 4.4 Tasks tab: tree render (indent by Depth, `◆` PR marker), `h/l`
...
- [x] 4.5 Detail panes (each a free-scroll viewport): task (fields + assignee + PR
