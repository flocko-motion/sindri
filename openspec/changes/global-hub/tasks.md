...
- [ ] 3.1 Drop `h.root`; make the Hub hold central paths + the one store. Per-agent maps (`agentLn`, `lifecycle`, `launchBuf`) key on `(project, name)`.
- [ ] 3.2 Rework `New`/`Serve`/`Close` for one global instance (no per-repo root); register a project on first request.
- [ ] 3.3 Rehome agent Claude-homes and per-agent sockets under `<state>/<repoTag>/`; resolve worktree paths to `<repo>/.worktrees/<agent>` via the registry.
- [ ] 3.4 Fold the project into macOS token derivation (`HMAC(secret, project+"\x00"+name)`) and resolve `token → (project, agent)`; scope Linux socket identity to `(project, agent)`.

## 4. Request routing (repo context on the wire)

- [ ] 4.1 Client: resolve repo root (cwd) and send it as `X-Sindri-Project` on repo-scoped calls; dial the one global socket. Thread through the ~36 client methods.
- [ ] 4.2 Hub server: middleware/helper that resolves the project from the header (host CLI/TUI) or from agent identity (agent socket/token), and passes it to handlers across the ~32 routes.
...
- [ ] 5.1 Split `BoardState`: global `Agents`/`PRs` tagged with `project`/repo; `Tasks` for the requested project. Update `/state` (takes selected repo) and `/events` (global agent/PR changes).
...
- [ ] 6.1 `sindri hub start/--bg/stop`: operate on the one global hub; replace `hub list` with `sindri hub status` (the running hub's pid/version/uptime + the projects it serves).
- [ ] 6.2 `coauthor`/`tui` bootstrap: keep auto-start of the global hub; ensure they register/select the cwd's repo.
- [ ] 6.3 Update user-facing strings that say "per-repo hub" / point at `.sindri/`.

## 7. Verification

- [ ] 7.1 Store tests: project scoping + `(project, name)` isolation (same name in two projects).
- [ ] 7.2 Token test: `(project, agent)` derivation + resolution; cross-project token rejected.
...
- [ ] 7.4 `make verify` green; `openspec validate global-hub` passes.
