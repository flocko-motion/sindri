...
### Requirement: A path in an item's detail is actionable

Where an item's detail names a filesystem path, it SHALL show that path in absolute form
and offer it as an actionable item: reachable by the detail's focus cursor, opening a shell
there when selected, and copied by the yank key. An agent's detail SHALL name its workspace
this way whenever that path resolves, independently of whether the agent has a PR open.
