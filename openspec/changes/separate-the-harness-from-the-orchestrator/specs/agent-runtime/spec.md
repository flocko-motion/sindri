...
The harness interface SHALL name only the agent and its box — liveness, idleness, context fill,
saying something, clearing, compacting, changing model, interrupting, starting. It SHALL NOT name a
task, a PR, a review, a backlog or a verdict, and SHALL NOT consult the orchestrator's rules to
decide whether to carry out a command it was given.
