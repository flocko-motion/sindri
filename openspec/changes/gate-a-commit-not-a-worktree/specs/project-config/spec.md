...
The key SHALL be validated like every other path key — absolute, escaping the repository
root, or naming a file that does not exist is invalid config, and fails loudly rather
than falling back.

A declared command OWNS the gate: when the key is set the built-in checks SHALL NOT also run.
The project's script is the thing that can run them itself — this repo's does, on the linter it just
built — so running both meant paying for the same checks twice on every gate in the fleet, and there
is no reading of "the project asked for this command" under which sindri also knows better.
...
#### Scenario: A declared gate is the only gate

- **GIVEN** `verify: scripts/verify.sh` in a Go project
- **WHEN** the gate runs
- **THEN** the built-in checks do not run beside it, and the script's result decides alone
